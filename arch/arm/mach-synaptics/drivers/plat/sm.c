...
static int syna_load_sm(void)
{
	...
	memcpy((void *)SOC_ITCM_BASE, __sm_fw_begin, __sm_fw_end - __sm_fw_begin);
...
static int syna_load_sm(void)
{
	...
	const char *pt_name = get_current_slot() ? FIRMWARE_B_NAME : FIRMWARE_A_NAME;
	...
	fb_mmc_flash_read(pt_name, header, MAX_PAGESIZE);
	...
	img_hdr = (void *)(header + PREPEND_IMAGE_INFO_SIZE);
