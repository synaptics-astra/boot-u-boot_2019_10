...
#define SPI_TX_MAX_DATA_LEN		(64 << 10) /* 64KiB */
#define SPI_RX_MAX_DATA_LEN		(64 << 10) /* 64KiB */
...
static int syna_spi_set_speed(struct udevice *bus, uint hz)
{
	...
	unsigned int base_clk = plat->ref_clock_hz;

	/* config SPI master speed */
	if (hz > base_clk)
		value = 1;
	else
		value = (base_clk + hz - 1000) / hz;
...
static int syna_spi_probe(struct udevice *bus)
{
	...
	set_transfer_mode(plat, 3);	/* half-duplex transfer mode */
